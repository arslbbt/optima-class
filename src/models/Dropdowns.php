...
class Dropdowns extends Model
{
    ...
    {
        $query = '';
        $types = isset($prop_types['type']) && !empty($prop_types['type']) ? $prop_types['type'] : [];
        ...
        $file = Functions::directory() . 'countries'.(!empty($model_type) ? $model_type : '').(!empty($types) ?  '_'.implode('-', $types) : '').'.json';
